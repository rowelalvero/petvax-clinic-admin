...
class ServicesController extends Controller
{
    ...
    public function index_data(Datatables $datatable, Request $request)
    {
        ...
        $datatable = $datatable->eloquent($query)
            ...
            ->orderColumn('category_id', function ($query, $order) {
                $query->orderByRaw(
                    '(SELECT name FROM categories WHERE categories.id = services.category_id LIMIT 1) ' . $order
                );
            })
    ...
    public function uniqueServices(Request $request)
    {
        ...
        $isUnique = true;
        if (!$serviceId) {
            $isUnique = Service::where('name', $service)->where('type', $service_type)
                ->doesntExist();
        }
