...
class EarningsController extends Controller
{
    ...
    public function index_data(DataTables $datatable)
    {
        ...
        return $datatable->eloquent($query)
            ...
            ->editColumn('total_service_amount', function ($data) {
                ...
                if (!empty($data['commission'])) {
                    foreach ($data['commission'] as $commission) {
                        $bookingData = Booking::where('id', $commission->commissionable_id)->with('bookingTransaction')->first();
                        if ($bookingData) {
                            $transaction = $bookingData->bookingTransaction;
                            $totalAmount = $bookingData->service_amount ?? 0;
            ...
            ->editColumn('total_tax_amount', function ($data) {
                $totalTaxAmount = 0;
                foreach ($data['commission'] as $commission) {
                    $bookingData = Booking::where('id', $commission->commissionable_id)
                        ->with('bookingTransaction')
                        ->first();
                    if ($bookingData && $bookingData->bookingTransaction) {
                        $transaction = $bookingData->bookingTransaction;
                        // Calculate tax amount
                        if (!empty($transaction->tax_percentage)) {
                            $taxes = json_decode($transaction->tax_percentage, true);
                            $totalTaxAmount += collect($taxes)->reduce(function ($carry, $tax) use ($bookingData) {
                                if ($tax['type'] === 'percentage') {
                                    return $carry + ($bookingData->service_amount * $tax['value'] / 100);
                                } elseif ($tax['type'] === 'fixed') {
                                    return $carry + $tax['value'];
                                }
                                return $carry;
                            }, 0);
                        }
                ...
                return Currency::format($totalTaxAmount);
            })
            ...
            ->editColumn('total_admin_earning', function ($data) {
                ...
                foreach ($data['commission'] as $commission) {
                    ...
                    if ($bookingData) {
                        ...
                        // Assuming you have a relationship or method to get employee earnings from booking data
                        if ($bookingData->employeeEarnings) {
                            $totalEmployeeEarnings += $bookingData->employeeEarnings->amount;
                        }
                    }
                }
                ...
                // Calculate totalAdminEarning by excluding tax amounts
                $totalAdminEarning = $totalServiceAmount - $totalPay - $totalEmployeeEarnings;
    ...
    public function edit($id, Request $request)
    {
        if ($request->commission_type == 'order') {
            $query = User::where('id', $id)->select('users.id', 'users.first_name', 'users.last_name', 'users.email', 'users.mobile')
                ->withCount([
                    'employeeOrder as totalOrders' => function ($q) {
    ...
    public function get_employee_commissions(Request $request)
    {
        ...
        if ($request->has('type') && $request->type != '' && $request->has('id') && $request->id != '') {
            ...
            $type = $request->type;
            $data = User::where('id', $request->id)->with([
                'commissions_data' => function ($query) use ($type) {
