<div class="d-flex gap-2 align-items-center">
    @if ($enable_push_notification == 1)
        <button type='button' data-assign-module="{{ $data->id }}" data-assign-target='#send_push_notification'
            data-assign-event='employee_assign' class='fs-4 text-info border-0 bg-transparent text-nowrap'
            data-bs-toggle="tooltip" title="Send Push Notification"> <i class="fa-regular fa-paper-plane"></i></button>
    @endif
    @hasPermission('edit_employee_password')
        <button type='button' data-assign-module="{{ $data->id }}" data-assign-target='#Employee_change_password'
            ...
            data-bs-toggle="tooltip" title="Change Password"><i class="icon-password"></i></button>
    @endhasPermission
